...
function createServer () {
  ...
  io.on('connection', socket => {
    ...
    socket.on('rooms:join', room => {
      const playersInRoom = io.sockets.adapter.rooms.get(room)?.size ?? 0
    ...
    socket.on('disconnect', () => {
      ...
      if (game) {
        ...
        else delete gameRooms[socket.roomName]
      }
    ...
    function updateLobby (roomId, players, playingPlayerId) {
      const data = { players, playingPlayerId }
  ...
  function serializeRooms () {
    const rooms = {}
    for (const [name, room] of io.sockets.adapter.rooms.entries()) {
      if (name.match(/^qtd:[24];id:[a-f0-9]{4}$/)) {
        rooms[name] = { size: room.size }
      }
    }
    return rooms
  }
  ...
  function updateRoomList () {
    io.sockets.emit('rooms:update', serializeRooms())
  }
